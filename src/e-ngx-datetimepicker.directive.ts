...
@Directive({
	selector: '[eNgxDTPicker][ngModel]',
	exportAs: 'eNgxDTPicker',
	providers: [NgModel]
})
export class ENgxDatetimepickerDirective implements ControlValueAccessor, OnInit, OnDestroy {
	...
	private initEvent() {
		...
		this.commonPicker().on('dp.update', (ev) => {
			const id: number = window.setTimeout(() => {
				...
				const currentDate: string = this.format(ev.viewDate.toDate(), this.commonFn('format')());
				this.writeValue(currentDate);
